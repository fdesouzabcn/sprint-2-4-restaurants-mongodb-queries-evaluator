...
db.restaurants.aggregate([{$match: {'grades.score': {$gt: 80, $lt: 100}}}, {$project:{ _id: 0, restaurant_id: 1, name: 1,grades: {$filter: {input: "$grades", as: "grade", cond:{ $and:[{ $gt:["$$grade.score", 80]},{ $lt: ["$$grade.score", 100]}]}}}}}]);
